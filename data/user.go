...
func (user *User) CreateSession() (session Session, err error) {
	...
	_,err = Db.Exec(statement,session.Uuid, user.Email, user.Id, session.CreatedAt)
	return
}
...
func (session *Session) DeleteByUUID() (err error) {
	...
	strSql := "delete from sessions where uuid = ?"
	//stmt, err := Db.Prepare(statement)
	//if err != nil {
	//	return
	//}
	...
	//return
	_,dbErr := Db.Exec(strSql,session.Uuid)
	if dbErr != nil {
		return
	}
	return
...
func (user *User) Create() (err error) {
	...
	strSql := "insert into users (uuid, name, email, password, created_at) values (?, ?, ?, ?, ?)"
	//stmt, err := Db.Prepare(statement)
	//if err != nil {
	//	return
	//}
	...
	_,dberr := Db.Exec(strSql, createUUID(),user.Name, user.Email, Encrypt(user.Password), time.Now())
	if dberr != nil {
		return dberr
	}
	return
...
func (user *User) Delete() (err error) {
	...
	//return
	_,dberr := Db.Exec(strSql, user.Id)
	if dberr != nil {
		return dberr
	}
	return
...
func (user *User) Update() (err error) {
	...
	strSql := "update users set name = ?,email=? where id = ?"
	//stmt, err := Db.Prepare(statement)
	//if err != nil {
	//	return
	//}
	...
	_,dberr := Db.Exec(strSql, user.Id, user.Name, user.Email)
	if dberr != nil {
		return dberr
	}
	return
